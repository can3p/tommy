...
## Wave 10 — the OpenAPI description, and the gate that keeps it true · 1 session

**Built:** an OpenAPI 3.1 description of `/api/v1`, generated from the route
table and the plugins' own declarations, served at `GET /api/v1/openapi.json`,
checked in at `docs/openapi.json`, regenerated by `make openapi`, and held to
the code by tests that fail when the two disagree.

**The gate was the deliverable; the document is a by-product.** A description
that is maintained by remembering to maintain it is wrong within two waves,
which is why most of this wave is machinery: a `RecordingMux` in `core/plugin`
so the server knows what a plugin mounted, `APIEndpoints()` on `Plugin` so it
knows what the plugin *meant* to mount, `plugintest.Conformance` comparing the
two per plugin, and a test in `plugins/all` comparing them across the whole
binary and against the checked-in file. Breaking one hl7 declaration on purpose
fails three tests in three different ways, which is how the gate was verified
rather than assumed.

**Rule 7 turned out to be half-applied.** "Every mounted route must be declared
and vice versa" had always covered a provider's *ingress* routes and never a
plugin's *API* routes, because until something read the declarations it did not
matter. It matters now. Extending the rule immediately caught three real
defects: as2 and chat each "declaring the same route twice" — which was really
`Pattern.Key()` dropping the method, fine for the ingress where two methods on
one path do not collide, wrong here where `GET` and `DELETE /messages` are
routine — and a mail endpoint description too short to say anything.

**Schemas are generated from the Go types, not written by hand.** Hand-written
schemas are precisely the drift this wave exists to prevent: a field added to a
response struct and forgotten in the document is invisible, and no test catches
what nobody wrote down. ~150 lines of reflection cover what the API actually
serves — structs, embedded structs inlined the way `encoding/json` inlines
them (the event envelope is an embedded `*event.Event` plus a `url`, and a
schema that referenced the embedded type would describe a shape the API never
sends), `[]byte` as base64, `any` as an unconstrained schema, `time.Time` as
date-time, and named struct types as shared components qualified by package so
`chat.MessageEnvelope` and `sms.MessageEnvelope` stay two different things.
Only struct types become components; a named string or map type would add a
component that says nothing its use site does not.

**Two response shapes became types** so the schema and the handler cannot
disagree: `HealthInfo`, which was an inline `map[string]any`, and `Error`,
which was an inline map in `writeError`.

**Running a real validator was worth more than reading the specification.**
`npx @redocly/cli lint` on the generated document found two errors that were
genuinely wrong rather than pedantic. `{path...}` is Go's wildcard syntax and
not OpenAPI's, so every `files` route described a parameter no generated client
would bind — the document now emits `{path}` and says in the parameter
description that it swallows slashes. And an API with no authentication has to
say so with an explicit empty `security`, or a reader is entitled to assume a
scheme was forgotten. The document now validates; the 14 remaining warnings are
`operation-4xx-response` on routes that genuinely cannot fail that way, and one
`info-license` on a repository that has no licence file. Neither was papered
over with an invented response.

**Deliberately not described:** the fake vendor endpoints. They are Mailjet's,
Twilio's and Slack's specifications rather than tommy's, and a partial copy of
somebody else's API is worse than none. The document says so in its own
description, and a test asserts both that the ingress routes are absent and
that the reason is stated.

**`info.version` is the API version, not the build version.** Putting the
binary's version there would rewrite the checked-in file on every release, for
a change no reader can act on, and the drift test would then fail for a reason
that has nothing to do with the API.

**`tommy openapi` ignores the local configuration on purpose.** It describes
every plugin the binary ships, because the checked-in document describes tommy
rather than one deployment of it; a `tommy.toml` that disables a plugin must not
silently shrink it. The *served* document is the opposite and describes only
what that instance enabled, which is the more useful answer when you are
looking at a running server.

**No plugin's user-facing behaviour changed**, which is why the plugin READMEs
gained only a pointer to the description rather than a rewrite: the routes they
document are the routes that were already there.
