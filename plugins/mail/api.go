...
func (p *Plugin) APIEndpoints() []plugin.APIEndpoint {
	list := append(plugin.CommonListParams(),
		plugin.APIParam{Name: "to", Description: "Substring of any recipient - To, Cc or Bcc."},
		plugin.APIParam{Name: "from", Description: "Substring of the sender."},
		plugin.APIParam{Name: "subject", Description: "Substring of the subject."},
		plugin.APIParam{Name: "has_attachments", Description: "Only messages with, or without, attachments.", Type: "boolean"},
	)
	return []plugin.APIEndpoint{
		...
		{Method: "GET", Path: "/messages/{id}/raw", Description: "The untouched request that produced the message; ?download=1 serves it as an .eml.",
			Query:    []plugin.APIParam{{Name: "download", Description: "Serve as a downloadable message/rfc822 attachment.", Type: "boolean"}},
			Produces: "text/plain"},
		{Method: "GET", Path: "/messages/{id}/attachments/{idx}", Description: "One attachment, streamed from the blob store with range support.",
			Query: []plugin.APIParam{
