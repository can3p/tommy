...
func (p *Plugin) APIEndpoints() []plugin.APIEndpoint {
	list := append(plugin.CommonListParams(),
		plugin.APIParam{Name: "message_type", Description: "The whole type (ADT^A01), just the code (ADT) or just the trigger event (A01)."},
		plugin.APIParam{Name: "control_id", Description: "MSH-10, the message control id."},
		plugin.APIParam{Name: "sending_application", Description: "MSH-3, the sending application."},
		plugin.APIParam{Name: "receiving_application", Description: "MSH-5, the receiving application."},
		plugin.APIParam{Name: "segment", Description: "Only messages carrying this segment, such as PID."},
	)
